...
REMEDIATION_CASES_V24: tuple[GoldCase, ...] = (
    GoldCase(
        case_id="X01",
        title="Suspected metastatic carcinoma with pending biopsy",
        target_failure_mode="Final uncertainty invariant after single canonicalization",
        narrative=(
    ...
    GoldCase(
        case_id="X02",
        title="Newly diagnosed AML with pending molecular studies",
        target_failure_mode="Explicit newly diagnosed state plus molecular missingness",
        narrative=(
            "A 64-year-old woman has newly diagnosed, bone-marrow-confirmed acute myeloid leukemia. ECOG is 1. "
            "FLT3, NPM1 and cytogenetic testing are pending. Induction therapy has not yet started."
        ),
    ...
    GoldCase(
        case_id="X03",
        title="Newly diagnosed MDS with planned azacitidine",
        target_failure_mode="Explicit new diagnosis with planned therapy separation",
        narrative=(
            "A 72-year-old woman has newly diagnosed, biopsy-confirmed myelodysplastic syndrome. Azacitidine is planned for next week but has not started. "
    ...
    GoldCase(
        case_id="X04",
        title="Relapsed DLBCL with multi-line chronology",
    ...
    GoldCase(
        case_id="X06",
        title="Suspected lymphoma with pending biopsy",
        target_failure_mode="Cross-field pathology reconciliation",
        narrative=(
            "A 58-year-old man has bulky cervical and mediastinal adenopathy. Lymphoma is suspected, but excisional biopsy is pending. "
    ...
    GoldCase(
        ...
        target_failure_mode="Progression canonicalization and exact provenance",
        narrative=(
            "A 67-year-old woman has follicular lymphoma previously treated with bendamustine plus rituximab. Current PET/CT demonstrates radiographic progression. ECOG is 1."
        ),
    ...
    GoldCase(
        case_id="X08",
        title="Remote metastatic breast cancer with newly diagnosed AML",
        target_failure_mode="Historical malignancy contamination guard",
        narrative=(
            "A 70-year-old woman had metastatic breast cancer treated in 2011 and has had no active breast cancer for many years. "
            "She now has newly diagnosed, bone-marrow-confirmed acute myeloid leukemia. ECOG is 1."
        ),
        expected_diagnosis="acute myeloid leukemia",
        expected_disease_state="newly diagnosed",
        expected_ecog="1",
    ...
    GoldCase(
        ...
        target_failure_mode="Repeated uncertainty invariant stress test",
        narrative=(
    ...
    GoldCase(
        ...
        target_failure_mode="Metastatic canonicalization",
        narrative=(
...
REMEDIATION_REPEAT_CASE_IDS_V24: tuple[str, ...] = ("X01", "X04", "X05", "X07", "X09", "X10")
REMEDIATION_REPEAT_COUNT_V24 = 3


def get_remediation_case_v24(case_id: str) -> GoldCase:

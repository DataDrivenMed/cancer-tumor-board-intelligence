...
from agents.extraction_v22 import extract_case_v22
...
from services.remediation_validation_v22 import (
    add_run_v22,
    aggregate_study_v22,
    build_remediation_run_payload_v22,
    load_latest_study_v22,
    new_remediation_study_v22,
    persist_study_v22,
    study_to_case_csv_v22,
)

st.set_page_config(page_title="Remediation Validation v2.2", page_icon="🧪", layout="wide")
...
def _compatible(study: dict, model_name: str, reasoning_effort: str) -> tuple[bool, str]:
    current = remediation_protocol_metadata_v22()
    actual = study.get("protocol") or {}
    if actual.get("remediation_fingerprint") != current.get("remediation_fingerprint"):
        return False, "Study fingerprint differs from the current frozen v2.2 configuration."
...
def _persist(study: dict) -> None:
    try:
        persist_study_v22(study, PROJECT_ROOT / "runtime_data" / "remediation_validation_v22")
...
def _preflight(protocol: dict, model_name: str, reasoning_effort: str) -> tuple[bool, list[str]]:
    ...
    if len(str(protocol.get("remediation_fingerprint") or "")) != 64:
        failures.append("v2.2 remediation fingerprint is missing or malformed.")
...
def _run_cases(cases, stream_name: str, model_token: str | None, model_name: str, reasoning_effort: str, protocol: dict) -> None:
    ...
    for idx, gold in enumerate(cases, start=1):
        ...
        document = parse_text(gold.narrative, document_id=f"V22-{gold.case_id}", filename=f"{gold.case_id}.txt")
        try:
            package = extract_case_v22(document=document, api_key=model_token or "local-no-auth", model=model_name, case_id=f"V22-{gold.case_id}")
            score = score_case_v22(gold, package)
            scores.append(score)
    ...
    payload = build_remediation_run_payload_v22(stream=stream_name, scores=scores, diagnostics=diagnostics, model_name=model_name, reasoning_effort=reasoning_effort, completed=failure is None and len(scores) == len(cases), failure=failure)
    payload["protocol"] = protocol
    st.session_state.remediation_v22_latest_payload = payload

    if failure is not None or len(scores) != len(cases):
        st.error("The batch is incomplete and was NOT added to the formal v2.2 study.")
        st.download_button("Download incomplete batch JSON", data=json.dumps(payload, indent=2, default=str).encode("utf-8"), file_name=f"remediation_v22_{stream_name}_incomplete.json", mime="application/json")
        return

    try:
        updated = add_run_v22(st.session_state.remediation_v22_study, payload, stream_name)
        st.session_state.remediation_v22_study = updated
        _persist(updated)
        st.success(f"Complete {stream_name} batch added to Remediation Validation v2.2.")
        st.rerun()
    except Exception as exc:
        st.error(f"Complete batch was not added because protocol validation failed: {exc}")
        st.download_button("Download rejected complete batch JSON", data=json.dumps(payload, indent=2, default=str).encode("utf-8"), file_name=f"remediation_v22_{stream_name}_rejected.json", mime="application/json")
...
st.title("Remediation Validation v2.2")
st.caption("Fresh frozen validation after v2.1: canonical disease state, diagnostic uncertainty, stage separation, exact provenance repair, treatment completeness")
...
with st.expander("Frozen v2.2 protocol", expanded=True):
    st.markdown(f"""
...
The S01-S12 cases are new for v2.2. Once baseline inference starts, do not modify extraction logic, scoring, cases, model, reasoning effort, or the frozen source set inside this study.
""")
...
if "remediation_v22_study" not in st.session_state:
    saved = load_latest_study_v22(PROJECT_ROOT / "runtime_data" / "remediation_validation_v22")
    if saved:
        ok, _ = _compatible(saved, model_name, reasoning_effort)
        st.session_state.remediation_v22_study = saved if ok else new_remediation_study_v22(model_name=model_name, reasoning_effort=reasoning_effort)
    else:
        st.session_state.remediation_v22_study = new_remediation_study_v22(model_name=model_name, reasoning_effort=reasoning_effort)

st.divider()
st.subheader("Restore study after reboot")
uploaded = st.file_uploader("Optional: restore Remediation Validation v2.2 JSON", type=["json"], key="remediation_v22_upload")
if uploaded is not None:
    try:
        ...
        if not ok:
            st.error(reason)
        elif st.button("Use uploaded v2.2 study"):
            st.session_state.remediation_v22_study = candidate
            _persist(candidate)
            st.rerun()
    except Exception as exc:
        st.error(f"Could not read study archive: {exc}")

study = st.session_state.remediation_v22_study
...
b.metric("Repeat runs", f"{summary['repeat_runs_completed']} / {REMEDIATION_REPEAT_COUNT_V22}")
...
study_csv = study_to_case_csv_v22(study).encode("utf-8")
d1, d2 = st.columns(2)
d1.download_button("Download Remediation v2.2 study (JSON)", data=study_json, file_name="remediation_validation_v22_study.json", mime="application/json", use_container_width=True)
d2.download_button("Download v2.2 case results (CSV)", data=study_csv, file_name="remediation_validation_v22_cases.csv", mime="text/csv", use_container_width=True)

st.divider()
st.subheader("Phase 1 • Fresh 12-case v2.2 baseline")
if study.get("baseline_run") is None:
    ack = st.checkbox("I understand this is a single-pass frozen 12-case baseline and I will not rerun individual cases to replace failures.", key="v22_baseline_ack")
    if st.button("Run v2.2 remediation baseline", type="primary", disabled=not ack or summary["safety_stop"]):
        _run_cases(REMEDIATION_CASES_V22, "baseline", model_token, model_name, reasoning_effort, protocol)
else:
    st.success("v2.2 fresh baseline complete. Do not rerun or replace it inside this study.")

st.divider()
st.subheader("Phase 2 • Frozen repeated subset")
repeat_cases = [get_remediation_case_v22(case_id) for case_id in REMEDIATION_REPEAT_CASE_IDS_V22]
...
elif summary["repeat_runs_completed"] >= REMEDIATION_REPEAT_COUNT_V22:
    st.success("All three repeat runs are complete.")
else:
    ack_repeat = st.checkbox(f"I understand Repeat {next_repeat} makes six frozen case executions and must not be selectively rerun.", key=f"v22_repeat_ack_{next_repeat}")
    if st.button(f"Run stochastic Repeat {next_repeat} of {REMEDIATION_REPEAT_COUNT_V22}", disabled=not ack_repeat):

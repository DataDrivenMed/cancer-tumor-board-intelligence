...
DEFAULT_STUDY_DIR = Path("runtime_data") / "remediation_validation_v21"
LATEST_STUDY_JSON = "latest_remediation_v21_study.json"
STUDY_SCHEMA_VERSION = "2.1"
BASELINE_EXECUTIONS = len(REMEDIATION_CASES)
REPEAT_EXECUTIONS = len(REMEDIATION_REPEAT_CASE_IDS) * REMEDIATION_REPEAT_COUNT
...
def expected_ids_for_stream(stream: str) -> tuple[str, ...]:
    if stream == "baseline":
        return tuple(case.case_id for case in REMEDIATION_CASES)
    if stream == "repeat":
        return REMEDIATION_REPEAT_CASE_IDS
    raise ValueError(f"Unknown remediation stream: {stream}")


def build_remediation_run_payload(
    *,
    stream: str,
    scores: list[Any],
...
def evaluate_run_payload(payload: dict[str, Any], stream: str) -> dict[str, Any]:
...
def validate_run(run: dict[str, Any], stream: str, protocol: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    expected_ids = expected_ids_for_stream(stream)
    if not run.get("completed"):
        errors.append("Only complete runs may be added to Remediation Validation v2.1.")
    ...
    if not run_fingerprint:
        errors.append("Run has no Remediation Validation v2.1 fingerprint.")
    elif run_fingerprint != expected_fingerprint:
        errors.append("Remediation fingerprint mismatch; results from different frozen configurations cannot be combined.")
    return errors


def new_remediation_study(*, model_name: str, reasoning_effort: str) -> dict[str, Any]:
    return {
        "schema_version": STUDY_SCHEMA_VERSION,
...
def _check_model(study: dict[str, Any], run: dict[str, Any]) -> None:
    if run.get("model_name") != study.get("model_name"):
        raise ValueError("Model changed during Remediation Validation v2.1; start a separate study.")
    if run.get("reasoning_effort") != study.get("reasoning_effort"):
        raise ValueError("Reasoning effort changed during Remediation Validation v2.1; start a separate study.")


def add_run(study: dict[str, Any], payload: dict[str, Any], stream: str) -> dict[str, Any]:
    evaluated = evaluate_run_payload(payload, stream)
    protocol = study.get("protocol") or remediation_protocol_metadata()
    errors = validate_run(evaluated, stream, protocol)
    if errors:
    ...
    if stream == "baseline":
        if updated.get("baseline_run") is not None:
            raise ValueError("The 12-case remediation baseline has already been run; it is single-pass by design.")
        updated["baseline_run"] = evaluated
    elif stream == "repeat":
        runs = updated.setdefault("repeat_runs", [])
        if len(runs) >= REMEDIATION_REPEAT_COUNT:
            raise ValueError("The remediation repeated subset is already complete.")
...
def aggregate_study(study: dict[str, Any]) -> dict[str, Any]:
    ...
    for case_id in REMEDIATION_REPEAT_CASE_IDS:
        case_rows = [
            row
            for run in study.get("repeat_runs", []) or []
            for row in run.get("case_results", [])
            if row.get("case_id") == case_id
        ]
        passes = sum(1 for row in case_rows if row.get("overall_pass"))
        repeat_stability[case_id] = {
            "passes": passes,
            "runs": len(case_rows),
            "failures": len(case_rows) - passes,
            "pass_rate": passes / len(case_rows) if case_rows else 0.0,
        }
    ...
    study_complete = (
        study.get("baseline_run") is not None
        and len(study.get("repeat_runs", []) or []) == REMEDIATION_REPEAT_COUNT

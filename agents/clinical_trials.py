...
def _age_in_years(value: str | None) -> float | None:
    """Parse ClinicalTrials.gov age strings into years for deterministic screening.

    This is a coarse hard-exclusion screen only. It does not establish eligibility.
    """
    text = _norm(value)
...
def _age_compatible(case_age: int | None, record) -> tuple[bool, str | None]:
    """Return whether a known case age is compatible with explicit registry bounds."""
    if case_age is None:
        return True, None
...
class ClinicalTrialsAgent:
    ...
    receive an investigational intervention. Explicit registry age bounds are used
    only as hard exclusion filters to prevent obviously incompatible candidates.
    """
    ...
    def run(self, case: CancerTumorBoardCase) -> ClinicalTrialsReport:
        ...
        for record in records:
            ...
            matches.append(
                TrialMatch(
                    ...
                    rationale=(
                        "Current ClinicalTrials.gov record has active recruitment status, overlaps the represented disease context, "
                        "and is not excluded by explicit registry age bounds. Eligibility has not been determined."
                    ),
        ...
        if age_excluded:
            warnings.append(
                f"Excluded {len(age_excluded)} otherwise disease-matched active trial record(s) because explicit registry age bounds were incompatible with the represented age."
            )

...
from qualification.remediation_protocol_v22 import remediation_protocol_metadata_v22
...
DEFAULT_STUDY_DIR = Path("runtime_data") / "remediation_validation_v22"
LATEST_STUDY_JSON = "latest_remediation_v22_study.json"
STUDY_SCHEMA_VERSION = "2.2"
BASELINE_EXECUTIONS = len(REMEDIATION_CASES_V22)
REPEAT_EXECUTIONS = len(REMEDIATION_REPEAT_CASE_IDS_V22) * REMEDIATION_REPEAT_COUNT_V22
...
def expected_ids_for_stream_v22(stream: str) -> tuple[str, ...]:
    if stream == "baseline":
        return tuple(case.case_id for case in REMEDIATION_CASES_V22)
    if stream == "repeat":
        return REMEDIATION_REPEAT_CASE_IDS_V22
    raise ValueError(f"Unknown v2.2 remediation stream: {stream}")


def build_remediation_run_payload_v22(*, stream: str, scores: list[Any], diagnostics: dict[str, dict[str, Any]], model_name: str, reasoning_effort: str, completed: bool, failure: dict[str, Any] | None = None) -> dict[str, Any]:
    return {
...
def evaluate_run_payload_v22(payload: dict[str, Any], stream: str) -> dict[str, Any]:
    expected_ids = expected_ids_for_stream_v22(stream)
...
def validate_run_v22(run: dict[str, Any], stream: str, protocol: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    expected_ids = expected_ids_for_stream_v22(stream)
    if not run.get("completed"):
        errors.append("Only complete runs may be added to Remediation Validation v2.2.")
    ...
    if not run_fingerprint:
        errors.append("Run has no Remediation Validation v2.2 fingerprint.")
    elif run_fingerprint != expected_fingerprint:
        errors.append("Remediation v2.2 fingerprint mismatch; different frozen configurations cannot be combined.")
    return errors


def new_remediation_study_v22(*, model_name: str, reasoning_effort: str) -> dict[str, Any]:
    return {
        "schema_version": STUDY_SCHEMA_VERSION,
...
def add_run_v22(study: dict[str, Any], payload: dict[str, Any], stream: str) -> dict[str, Any]:
    evaluated = evaluate_run_payload_v22(payload, stream)
    protocol = study.get("protocol") or remediation_protocol_metadata_v22()
    errors = validate_run_v22(evaluated, stream, protocol)
    if errors:
        raise ValueError(" ".join(errors))
    if evaluated.get("model_name") != study.get("model_name"):
        raise ValueError("Model changed during Remediation Validation v2.2; start a separate study.")
    if evaluated.get("reasoning_effort") != study.get("reasoning_effort"):
        raise ValueError("Reasoning effort changed during Remediation Validation v2.2; start a separate study.")
    updated = json.loads(json.dumps(study, default=str))
    if stream == "baseline":
        if updated.get("baseline_run") is not None:
            raise ValueError("The v2.2 12-case baseline has already been run; it is single-pass by design.")
        updated["baseline_run"] = evaluated
    elif stream == "repeat":
        runs = updated.setdefault("repeat_runs", [])
        if len(runs) >= REMEDIATION_REPEAT_COUNT_V22:
            raise ValueError("The v2.2 repeated subset is already complete.")
...
def aggregate_study_v22(study: dict[str, Any]) -> dict[str, Any]:
    ...
    for case_id in REMEDIATION_REPEAT_CASE_IDS_V22:
    ...
    study_complete = study.get("baseline_run") is not None and len(study.get("repeat_runs", []) or []) == REMEDIATION_REPEAT_COUNT_V22 and total == PLANNED_EXECUTIONS
    if total == 0:
...
def study_to_case_csv_v22(study: dict[str, Any]) -> str:
    stream = io.StringIO()
...
def persist_study_v22(study: dict[str, Any], study_dir: Path = DEFAULT_STUDY_DIR) -> Path:
...
def load_latest_study_v22(study_dir: Path = DEFAULT_STUDY_DIR) -> dict[str, Any] | None:

...
from agents.extraction_v23 import extract_case_v23
from qualification.remediation_cases_v23 import (
    ...
    get_remediation_case_v23,
)
from qualification.remediation_protocol_v23 import (
    assert_remediation_suite_shape_v23,
    remediation_protocol_metadata_v23,
)
from qualification.scoring_v23 import score_case_v23
from services.document_parser import parse_text
from services.remediation_validation_v23 import (
    add_run_v23,
    aggregate_study_v23,
    build_remediation_run_payload_v23,
    load_latest_study_v23,
    new_remediation_study_v23,
    persist_study_v23,
    study_to_case_csv_v23,
)

st.set_page_config(page_title="Remediation Validation v2.3", page_icon="🧪", layout="wide")
...
def _compatible(study: dict, model_name: str, reasoning_effort: str) -> tuple[bool, str]:
    current = remediation_protocol_metadata_v23()
    actual = study.get("protocol") or {}
    if actual.get("remediation_fingerprint") != current.get("remediation_fingerprint"):
        return False, "Study fingerprint differs from the current frozen v2.3 configuration."
...
def _persist(study: dict) -> None:
    try:
        persist_study_v23(study, PROJECT_ROOT / "runtime_data" / "remediation_validation_v23")
...
def _preflight(protocol: dict, model_name: str, reasoning_effort: str) -> tuple[bool, list[str]]:
    ...
    if len(str(protocol.get("remediation_fingerprint") or "")) != 64:
        failures.append("v2.3 remediation fingerprint is missing or malformed.")
...
def _run_cases(cases, stream_name: str, model_token: str | None, model_name: str, reasoning_effort: str, protocol: dict) -> None:
    ...
    for idx, gold in enumerate(cases, start=1):
        ...
        document = parse_text(gold.narrative, document_id=f"V23-{gold.case_id}", filename=f"{gold.case_id}.txt")
        try:
            package = extract_case_v23(document=document, api_key=model_token or "local-no-auth", model=model_name, case_id=f"V23-{gold.case_id}")
            score = score_case_v23(gold, package)
            scores.append(score)
    ...
    payload = build_remediation_run_payload_v23(stream=stream_name, scores=scores, diagnostics=diagnostics, model_name=model_name, reasoning_effort=reasoning_effort, completed=failure is None and len(scores) == len(cases), failure=failure)
    payload["protocol"] = protocol
    st.session_state.remediation_v23_latest_payload = payload

    if failure is not None or len(scores) != len(cases):
        st.error("The batch is incomplete and was NOT added to the formal v2.3 study.")
        st.download_button("Download incomplete batch JSON", data=json.dumps(payload, indent=2, default=str).encode("utf-8"), file_name=f"remediation_v23_{stream_name}_incomplete.json", mime="application/json")
        return

    try:
        updated = add_run_v23(st.session_state.remediation_v23_study, payload, stream_name)
        st.session_state.remediation_v23_study = updated
        _persist(updated)
        st.success(f"Complete {stream_name} batch added to Remediation Validation v2.3.")
        st.rerun()
    except Exception as exc:
        st.error(f"Complete batch was not added because protocol validation failed: {exc}")
        st.download_button("Download rejected complete batch JSON", data=json.dumps(payload, indent=2, default=str).encode("utf-8"), file_name=f"remediation_v23_{stream_name}_rejected.json", mime="application/json")
...
st.title("Remediation Validation v2.3")
st.caption("Fresh frozen validation after v2.2 safety stop: deterministic missingness reconciliation and exact uncertain-diagnosis provenance repair")
...
with st.expander("Frozen v2.3 protocol", expanded=True):
    st.markdown(f"""
...
W01-W12 are fresh v2.3 cases. Once baseline inference starts, do not modify extraction logic, scoring, cases, model, reasoning effort, or the frozen source set inside this study.
""")
...
if "remediation_v23_study" not in st.session_state:
    saved = load_latest_study_v23(PROJECT_ROOT / "runtime_data" / "remediation_validation_v23")
    if saved:
        ok, _ = _compatible(saved, model_name, reasoning_effort)
        st.session_state.remediation_v23_study = saved if ok else new_remediation_study_v23(model_name=model_name, reasoning_effort=reasoning_effort)
    else:
        st.session_state.remediation_v23_study = new_remediation_study_v23(model_name=model_name, reasoning_effort=reasoning_effort)

st.divider()
st.subheader("Restore study after reboot")
uploaded = st.file_uploader("Optional: restore Remediation Validation v2.3 JSON", type=["json"], key="remediation_v23_upload")
if uploaded is not None:
    try:
        ...
        if not ok:
            st.error(reason)
        elif st.button("Use uploaded v2.3 study"):
            st.session_state.remediation_v23_study = candidate
            _persist(candidate)
            st.rerun()
    except Exception as exc:
        st.error(f"Could not read study archive: {exc}")

study = st.session_state.remediation_v23_study
summary = aggregate_study_v23(study)

st.divider()
st.subheader("Study status")
a, b, c, d, e = st.columns(5)
a.metric("Baseline", "DONE" if summary["baseline_complete"] else "PENDING")
b.metric("Repeat runs", f"{summary['repeat_runs_completed']} / {REMEDIATION_REPEAT_COUNT_V23}")
...
study_csv = study_to_case_csv_v23(study).encode("utf-8")
d1, d2 = st.columns(2)
d1.download_button("Download Remediation v2.3 study (JSON)", data=study_json, file_name="remediation_validation_v23_study.json", mime="application/json", use_container_width=True)
d2.download_button("Download v2.3 case results (CSV)", data=study_csv, file_name="remediation_validation_v23_cases.csv", mime="text/csv", use_container_width=True)

st.divider()
st.subheader("Phase 1 • Fresh 12-case v2.3 baseline")
if study.get("baseline_run") is None:
    ack = st.checkbox("I understand this is a single-pass frozen 12-case baseline and I will not rerun individual cases to replace failures.", key="v23_baseline_ack")
    if st.button("Run v2.3 remediation baseline", type="primary", disabled=not ack or summary["safety_stop"]):
        _run_cases(REMEDIATION_CASES_V23, "baseline", model_token, model_name, reasoning_effort, protocol)
else:
    st.success("v2.3 fresh baseline complete. Do not rerun or replace it inside this study.")

st.divider()
st.subheader("Phase 2 • Frozen repeated subset")
repeat_cases = [get_remediation_case_v23(case_id) for case_id in REMEDIATION_REPEAT_CASE_IDS_V23]
...
elif summary["repeat_runs_completed"] >= REMEDIATION_REPEAT_COUNT_V23:
    st.success("All three repeat runs are complete.")
else:
    ack_repeat = st.checkbox(f"I understand Repeat {next_repeat} makes six frozen case executions and must not be selectively rerun.", key=f"v23_repeat_ack_{next_repeat}")
    if st.button(f"Run stochastic Repeat {next_repeat} of {REMEDIATION_REPEAT_COUNT_V23}", disabled=not ack_repeat):
